...
original <- read.csv("complete_renewable_energy_dataset.csv")
data <- original|> group_by(Country, Year, Energy.Type, Renewable.Energy.Education.Programs, Government.Policies) |> summarise(Total.Production = sum(Production..GWh.), Electricity.Prices = mean(Electricity.Prices), R.D.Expenditure = sum(R.D.Expenditure), CO2.Emissions = sum(CO2.Emissions), Investments..USD. = sum(Investments..USD.), Public.Awareness = mean(Public.Awareness), Educational.Level = mean(Educational.Level), Export = sum(Energy.Exports), Import = sum(Energy.Imports), Renewable.Energy.Jobs = sum(Renewable.Energy.Jobs), Renewable.Energy.Patents = sum(Renewable.Energy.Patents), Corruption.Perception.Index = mean(Corruption.Perception.Index)) %>% 
