...
```{r}
kable(head(most_important, 10), "simple")
```
...
```{r warning=FALSE}
annual <- df %>% group_by(Year, Country) %>% summarise(avg_electricity_price = mean(Electricity.Prices), avg_production = sum(Production..GWh.), avg_investment = sum(Investments..USD.), avg_CO2_emissions = sum(CO2.Emissions))
annual
```
